...
//задать диапазон по X дл€ вывода (номера столбцов пикселей начала и конца вывода)
...
//задать диапазон по Y дл€ вывода (номера строк пикселей начала и конца)
...
lcd_x_band(8*x, 127);
 ...
 sym_code=text[posit]-0x20;
  for (char_column=0; char_column<(CHAR_ARRAY_NAME[sym_code][0]); char_column++) { //цикл отрисовки символа по столбцам, ширина символа беретс€ из массива ascii
   for (column_pix=0;column_pix<8;column_pix++){
